...
def load_data():
    ...
    Note: groupid and location are one-to-one matched
    '''
...
def add_abnomal_data(df_raw, SNR, percentage):
    ...
    1. Randomly choose percentage rows to add noise
    2. Generate the nosiy sample 'noise' for  'current', 'voltage','phase'
    3. For the selected value(row, :) = value(row, :) + noise(SNR)
    '''
    ...
    noise = np.random.randn(df3.shape[0],df3.shape[1]) 	
    noise = noise-np.mean(noise)     							    
    signal_power = np.linalg.norm( df3 - df3.mean() )**2 / df3.size	
...
def get_statistics(df_raw, df_noisy):
    ...
    Draw three figures with each plotting a feature's raw-data curve and raw+noisy-data curve
    '''
...
def split_data(df_noisy, ratio):
    '''
    For each group id, split it as training and test data.
    With the ratio(train.shape[0] / test.shape[0]) along timestamp, the top part is train while the bottom is test
    '''
    train = {}
    test = {}
...
def abnormal_detection(train, test, nu):
    ...
    for key in sorted(test.keys()):
        ...
        test[key].to_csv('test_goupid{}.csv'.format(key))
    return result
